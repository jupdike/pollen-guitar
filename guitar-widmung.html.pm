#lang pollen


◊chord[Ab    (frets x 0 2 2 2 0)]
◊chord[Db    (frets x x 0 2 3 2)]
◊chord[Gb7   (frets 3 5 3 4 3 3)]
◊chord[Bbm   (frets x 2 4 4 3 2)]
◊chord[Eb7   (frets 0 2 0 1 0 0)]
◊chord[Ab7   (frets x 0 2 0 2 0)]
◊chord[Ab7/E (frets 0 0 2 0 2 3)]

◊title{Widmung by Robert Schumann}

◊desc{An attempt at an arrangement for guitar. Original key of Ab, tune guitar to Eb standard tuning (every string down a semitone). Capo fourth, fifth, sixth, or seventh fret to play in a different key for baritone vocal part. Original key of Ab has low note of B. Down 5 semitones (Key of Eb) is low note of Gb, down six (Key of D) is low note of F.}

◊song[
 ◊row[Ab  _ _ _   _ _  Ab _ _ _ _ _]
 ◊row[Ab  _ _ _   _ _  Db _ _ Gb7 _ _]
 ◊row[Bbm _ _  _ Db  _ Ab _ _ _   Eb7   _]
 ◊row[Eb7 _ Ab _ Ab7 _ Db _ _ _   Ab7/E _]
 ; TODO
]










◊;items{
◊;item{You ◊b{wanted} it — you ◊i{got} it.}
◊;item{◊link["https://google.com/search?q=racket"]{search for Racket}}  }
